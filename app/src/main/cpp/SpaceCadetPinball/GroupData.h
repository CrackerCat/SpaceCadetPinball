...
struct EntryData {
    ...
    EntryData(FieldTypes entryType, char *buffer) : EntryType(entryType), FieldSize(-1),
                                                    Buffer(buffer) {
    }
...
private:
    ...
    static void
    SplitSplicedBitmap(const gdrv_bitmap8 &srcBmp, gdrv_bitmap8 &bmp, zmap_header_type &zMap);
