...
void loader::loadfrom(DatFile *datFile) {
    ...
    for (auto groupIndex = 0; groupIndex < static_cast<int>(datFile->Groups.size()); ++groupIndex) {
        auto value = reinterpret_cast<int16_t *>(datFile->field(groupIndex,
    ...
    if (!sound_list[soundIndex].Loaded && !sound_list[soundIndex].WavePtr) {
        ...
        if (soundGroupId > 0 && !pinball::quickFlag) {
            ...
            if (value && *value == 202) {
                ...
                auto sampleCount = wavHeader.data_size /
                                   (wavHeader.channels * (wavHeader.bits_per_sample / 8.0));
                sound_list[soundIndex].Duration = static_cast<float>(sampleCount /
...
short loader::query_visual_states(int groupIndex) {
    ...
    if (groupIndex < 0)
        return error(0, 17);
    auto shortArr = reinterpret_cast<int16_t *>(loader_table->field(groupIndex,
...
float
loader::query_float_attribute(int groupIndex, int groupIndexOffset, int firstValue, float defVal) {
    if (groupIndex < 0) {
        error(0, 22);
        return NAN;
    }
...
int loader::material(int groupIndex, visualStruct *visual) {
    ...
    if (*shortArr != 300)
        return error(3, 21);
    auto floatArr = reinterpret_cast<float *>(loader_table->field(groupIndex,
                                                                  FieldTypes::FloatArray));
    if (!floatArr)
        return error(11, 21);
...
int loader::query_visual(int groupIndex, int groupIndexOffset, visualStruct *visual) {
    ...
    auto shortArr = reinterpret_cast<int16_t *>(loader_table->field(stateId,
                                                                    FieldTypes::ShortArray));
    if (shortArr) {
