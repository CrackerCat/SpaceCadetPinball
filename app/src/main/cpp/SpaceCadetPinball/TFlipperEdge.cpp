...
TFlipperEdge::TFlipperEdge(TCollisionComponent *collComp, char *activeFlag,
                           unsigned int collisionGroup, TPinballTable *table,
                           vector_type *origin, vector_type *vecT1, vector_type *vecT2,
...
void TFlipperEdge::EdgeCollision(TBall *ball, float coef) {
    ...
    if (circlebase.RadiusSq * 1.01f < distance)
        ...
        elasticity = Elasticity;
    maths::basic_collision(ball, &NextBallPosition, &CollisionDirection, elasticity, Smoothness,
                           1000000000.0, 0.0);
}
