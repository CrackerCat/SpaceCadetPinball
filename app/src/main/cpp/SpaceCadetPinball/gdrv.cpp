...
gdrv_bitmap8::gdrv_bitmap8(const dat8BitBmpHeader &header) {
    ...
    if (BitmapType == BitmapTypes::Spliced) {
        ...
        if (BitmapType == BitmapTypes::RawBitmap)
            assertm(Width % 4 == 0 || header.IsFlagSet(bmp8Flags::RawBmpUnaligned),
...
void
gdrv::fill_bitmap(gdrv_bitmap8 *bmp, int width, int height, int xOff, int yOff, uint8_t fillChar) {
...
void gdrv::copy_bitmap(gdrv_bitmap8 *dstBmp, int width, int height, int xOff, int yOff,
                       gdrv_bitmap8 *srcBmp,
...
void
gdrv::copy_bitmap_w_transparency(gdrv_bitmap8 *dstBmp, int width, int height, int xOff, int yOff,
