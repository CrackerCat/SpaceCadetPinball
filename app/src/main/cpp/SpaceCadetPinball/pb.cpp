        ...
        if (ball->ActiveFlag != 0) {
            ...
            if (collComp) {
                ...
                if (MainTable->ActiveFlag) {
                    ...
                    ball->Speed = maths::normalize_2d(&ball->Acceleration);
                    ball->InvAcceleration.X =
                            ball->Acceleration.X == 0.0f ? 1.0e9f : 1.0f / ball->Acceleration.X;
                    ball->InvAcceleration.Y =
                            ball->Acceleration.Y == 0.0f ? 1.0e9f : 1.0f / ball->Acceleration.Y;
                }
    ...
    if (!MainTable->CheatsUsed) {
        for (auto i = 0; i < playerCount; ++i) {
            ...
            if (position >= 0) {
                ...
                high_score::show_and_set_high_score_dialog(highscore_table, scores[i], position,
                                                           String1);
            }
        }
    }
...
bool pb::chk_highscore() {
    ...
    for (int i = playerIndex;
         high_score::get_score_position(highscore_table,
                                        MainTable->PlayerScores[i].ScoreStruct->Score) < 0;
         --i) {
        if (--playerIndex < 0)
            return false;
    }
