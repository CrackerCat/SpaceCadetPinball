...
//	ImGui::GetIO().FontGlobalScale = get_float("UI Scale", 1.0f);
    ...
    Options.FramesPerSecond = std::min(MaxFps,
                                       std::max(MinUps, get_int("Frames Per Second", DefFps)));
    Options.UpdatesPerSecond = std::min(MaxUps,
                                        std::max(MinUps, get_int("Updates Per Second", DefUps)));
    ...
    Options.SoundChannels = std::min(MaxSoundChannels,
                                     std::max(MinSoundChannels, Options.SoundChannels));
...
void options::toggle(Menu1 uIDCheckItem) {
    switch (uIDCheckItem) {
        ...
        case Menu1::FourPlayers:
            Options.Players =
                    static_cast<int>(uIDCheckItem) - static_cast<int>(Menu1::OnePlayer) + 1;
            break;
