...
void score::load_msg_font(const char *lpName) {
    ...
    auto gapArray = reinterpret_cast<int16_t *>(pb::record_table->field(groupIndex,
                                                                        FieldTypes::ShortArray));
    if (gapArray)
...
void score::erase(scoreStruct *score, int blitFlag) {
    if (score) {
        if (score->BackgroundBmp)
            ...
            gdrv::fill_bitmap(render::vscreen, score->Width, score->Height, score->OffsetX,
                              score->OffsetY, 0);
    }
}
        ...
        if (score->Score >= 0) {
            ...
            for (ptrdiff_t index = strlen(scoreBuf) - 1; index >= 0; index--) {
                ...
                if (render::background_bitmap)
                    gdrv::copy_bitmap_w_transparency(render::vscreen, width, height, x, y, bmp, 0,
                                                     0);
                else
        ...
        *str = 0;
        ...
        if (score / 1000000000 <= 0) {
            if (scoreMillions <= 0) {
                ...
                snprintf(str, 36, "%d,%03d,%03d", scoreMillions, score % 1000000 / 1000,
                         score % 1000);
            }
        } else {
            snprintf(
