...
TFlipper::TFlipper(TPinballTable *table, int groupIndex) : TCollisionComponent(table, groupIndex,
                                                                               false) {
    visualStruct visual{};
    ...
    auto origin = reinterpret_cast<vector_type *>(loader::query_float_attribute(groupIndex, 0,
                                                                                800));
    ...
    if (flipperEdge) {
        ExtendAnimationFrameTime =
                flipperEdge->ExtendTime / static_cast<float>(ListBitmap->size() - 1);
        RetractAnimationFrameTime =
                flipperEdge->RetractTime / static_cast<float>(ListBitmap->size() - 1);
    }
    BmpIndex = 0;
    InputTime = 0.0;
}
...
void TFlipper::Collision(TBall *ball, vector_type *nextPosition, vector_type *direction, float coef,
                         TEdgeSegment *edge) {
}
