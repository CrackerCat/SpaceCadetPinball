...
int winmain::WinMain(LPCSTR lpCmdLine) {
    ...
    if (SDL_Init(SDL_INIT_EVERYTHING) < 0) {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Could not initialize SDL2", SDL_GetError(),
                                 nullptr);
        return 1;
    }
//	BasePath = SDL_GetBasePath();
    ...
    MainWindow = window;
    if (!window) {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Could not create window", SDL_GetError(),
                                 nullptr);
        return 1;
    }
    ...
    Renderer = renderer;
    if (!renderer) {
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Could not create renderer", SDL_GetError(),
                                 window);
        return 1;
    }
...
//				ImGuiSDL::Render(ImGui::GetDrawData());
            ...
            auto targetTimeDelta =
                    TargetFrameTime - DurationMs(updateEnd - frameStart) - sleepRemainder;
